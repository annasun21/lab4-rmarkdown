...
```{r}
# income height data
ih <- read_delim("income-height.csv")
```
...
This dataset contians `r nrow(ih)` rows and `r ncol(ih)` columns.

4.  List the variable names of the dataset, separated by comma, using a
    complete sentence. Hint: check out function str_flatten_comma() in
    stringr library.

The variables in this dataset are:
`r str_flatten_comma(names(ih), last = ", and ")`.

5.  How many missings are there in variables income, height, age and
    sex? Again, answer in a complete sentence, injecting the numbers
    into the text like There are 5 missings in variable "age", 4
    missings in .... 2 Do taller people earn more?

There are `r sum(is.na(ih$income))` missing in income, `r sum(is.na(ih$height))` missing in height, `r sum(is.na(ih$age))` missing in age, and `r sum(is.na(ih$sex))` missing in sex

6.  Find the average height in data. Answer in a complete sentence,
    round the result as appro- priate. 1
7.  Compute the average income for those who are shorter than the
...
8.  But does the difference also hold if we do it separately by sex? Now
    ...
    income.
9.  Now comment your results in a sentence--something like "short women
...
10. Make a scatterplot where you plot height versus income. Do not
